...
function lifecal() {
	...
	var fm = document.getElementById('fm').value; //平均負荷
	var fw = document.getElementById('fw').value; //負荷係數
	var nm = document.getElementById('nm').value; //平均轉速
...
function renew(input) { //更新select
	...
	for (let i = 0; i < mydata.length; i++) {
		if (choosenut==mydata[i].螺帽型式 && lead == mydata[i].導程) { //外徑由螺帽型式及導程決定
			if (input==0) { //若當前為第1階段則判定螺桿外徑
				...
				}else if (choosebd.value==mydata[i].鋼珠直徑) { //若是第3階段則加上鋼珠直徑決定循環圈數
	...
	if (input<2) { //若不是第3階段則繼續下一階段
		renew(input+1)
	}else{
		showpara();//若是第3階段則根據規格顯示參數
	}
}

function showpara() { //根據螺帽規格顯示參數
